      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_.8fr]">
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-950">Solana Mobile: Proof of Care</h2>
              <p className="mt-1 text-sm text-slate-600">Integración ética para hackathon: Mobile Wallet Adapter + badge educativo + hash opcional de integridad.</p>
            </div>
          </div>
          <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50/90 p-4 text-sm leading-6 text-slate-700 shadow-inner">
            La wallet puede servir para autenticación pseudónima o para emitir un badge educativo en Devnet. La evidencia, historias, ubicación, contactos o nivel de riesgo nunca deben guardarse en blockchain. Solo una huella/hash verificable si la usuaria lo decide.
          </div>
          <Button onClick={() => setConnected((value) => !value)} className="mt-5">{connected ? "Desconectar wallet demo" : "Conectar wallet demo"}</Button>
        ...
        <Card className={cx("relative overflow-hidden", allDone && connected ? "border-teal-200 bg-teal-50/90" : "")}>
          ...
          <div className="relative">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">Reconocimiento simbólico por completar aprendizaje preventivo. Debe ser no transferible o de bajo incentivo para evitar gamificar el dolor.</p>
            <div className="mt-5 rounded-[1.5rem] border border-slate-200 bg-white/80 p-5 text-center shadow-sm">
              ...
              <p className="mt-1 text-xl font-black text-slate-950">{connected && allDone ? "Listo para emitir en Devnet" : "Pendiente"}</p>
            </div>
          </div>
        </Card>
      </div>
